...
def cargar_datos():
    try:
        df_final = pd.read_csv('baires_final.csv')
        df_final.drop(columns=['url_google_maps'],inplace = True)
        df_final.rename(columns={'id':'ID de registro','name':'nombre_unificado','address':'direccion_unificada'}, inplace=True)
...
def crear_hotspots_downtown(df, radio_max_km=2.0, min_puntos=5, max_zonas=5):
    try:
        ...
        centro_cdmx_lat = -34.6037
        centro_cdmx_lng = -58.3816
